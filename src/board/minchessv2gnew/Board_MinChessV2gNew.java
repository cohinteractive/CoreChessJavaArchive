...
import com.ohinteractive.minchessv2gnew.util.Fen;
...
import com.ohinteractive.minchessv2gnew.util.Zobrist;

public class Board_MinChessV2gNew {
    ...
    public static boolean isPlayerInCheck(long[] board, int player) {
        long bitboard = board[Piece.KING | (player << 3)];
        return isSquareAttackedByPlayer(board, B.LSB[(int) (((bitboard & -bitboard) * B.DB) >>> 58)], 1 ^ player);
    ...
    public static String toString(long[] board) {
        String string = "";
		string += "Player: " + (player(board) == 0 ? "White" : "Black");
